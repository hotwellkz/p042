# 🚀 Финальная инструкция по деплою

## ✅ Что уже готово:

1. ✅ Все скрипты деплоя созданы и закоммичены в репозиторий
2. ✅ Backend настроен на прослушивание `0.0.0.0:8080`
3. ✅ `BACKEND_URL` автоматически настраивается на `http://185.104.248.130:5001`
4. ✅ Медиа-роуты поддерживают Range-запросы
5. ✅ Документация создана

## 📋 Шаги для выполнения деплоя:

### Шаг 1: Проверка готовности (опционально)

```bash
cd backend
bash check_deploy_ready.sh
```

### Шаг 2: Настройка VPS (если ещё не сделано)

**На вашем компьютере:**
```bash
# Скопируйте скрипт на VPS
scp backend/vps/synology-port-forward.sh root@185.104.248.130:/root/
```

**На VPS:**
```bash
ssh root@185.104.248.130
chmod +x synology-port-forward.sh
sudo ./synology-port-forward.sh
```

**Проверка:**
```bash
# На VPS
curl -I http://127.0.0.1:5001/health
```

### Шаг 3: Деплой на Synology

**Вариант A: Автоматический деплой с вашего компьютера**

```bash
cd backend
export SYNO_HOST="hotwell.synology.me"  # или IP адрес Synology
export SYNO_USER="admin"                 # ваш пользователь
chmod +x deploy_synology_auto.sh
./deploy_synology_auto.sh
```

Скрипт автоматически:
- Подключится к Synology по SSH
- Скопирует код из GitHub
- Установит зависимости
- Скомпилирует TypeScript
- Настроит .env
- Запустит через pm2
- Настроит автозапуск

**Вариант B: Деплой прямо на Synology**

```bash
# Подключитесь к Synology
ssh admin@hotwell.synology.me

# Выполните команды
cd /volume1/shortsai
git clone https://github.com/hotwellkz/p041.git app
cd app/backend
chmod +x deploy_to_synology_production.sh
sudo ./deploy_to_synology_production.sh
```

### Шаг 4: Проверка работоспособности

**На Synology:**
```bash
# Локальная проверка
curl http://127.0.0.1:8080/health

# Проверка pm2
pm2 status
pm2 logs shortsai-backend
```

**Через VPS (публичный доступ):**
```bash
# С любого компьютера в интернете
curl http://185.104.248.130:5001/health

# Должен вернуть: {"ok":true}
```

**Проверка медиа-роута:**
```bash
# Проверка Range-запросов (для Blotato)
curl -I -H "Range: bytes=0-1023" http://185.104.248.130:5001/api/media/test/test/test.mp4
# Должен вернуть: 206 Partial Content
```

## 🔧 Управление после деплоя:

```bash
# Статус
pm2 status

# Логи
pm2 logs shortsai-backend

# Перезапуск
pm2 restart shortsai-backend

# Остановка
pm2 stop shortsai-backend

# Мониторинг
pm2 monit
```

## 📝 Важные файлы:

- **Автоматический деплой**: `backend/deploy_synology_auto.sh`
- **Деплой на Synology**: `backend/deploy_to_synology_production.sh`
- **Настройка VPS**: `backend/vps/synology-port-forward.sh`
- **Проверка готовности**: `backend/check_deploy_ready.sh`
- **Документация**: `backend/DEPLOY_PRODUCTION.md`

## ⚙️ Конфигурация:

После деплоя проверьте `.env` на Synology:

```bash
cat /volume1/shortsai/app/backend/.env | grep BACKEND_URL
# Должно быть: BACKEND_URL=http://185.104.248.130:5001
```

## 🐛 Устранение неполадок:

### Backend не отвечает:

1. Проверьте статус pm2:
   ```bash
   pm2 status
   ```

2. Проверьте логи:
   ```bash
   pm2 logs shortsai-backend --lines 50
   ```

3. Проверьте порт:
   ```bash
   netstat -tuln | grep 8080
   ```

### Не работает через VPS:

1. Проверьте VPN туннель:
   ```bash
   # На VPS
   ping 10.8.0.2
   ```

2. Проверьте правила iptables:
   ```bash
   # На VPS
   sudo iptables -t nat -L PREROUTING -n -v
   ```

3. Проверьте firewall на Synology:
   - Control Panel → Security → Firewall
   - Убедитесь, что порт 8080 разрешён для VPN сети

### Blotato не может получить доступ:

1. Проверьте `BACKEND_URL`:
   ```bash
   # На Synology
   grep BACKEND_URL /volume1/shortsai/app/backend/.env
   ```

2. Проверьте доступность из интернета:
   ```bash
   curl -I http://185.104.248.130:5001/health
   ```

3. Проверьте логи Blotato:
   ```bash
   pm2 logs shortsai-backend | grep "BlottataLocalFileProcessor"
   ```

## 📚 Дополнительная документация:

- **Подробная инструкция**: `backend/DEPLOY_PRODUCTION.md`
- **Быстрый старт**: `backend/QUICK_START_PRODUCTION.md`
- **Корневая инструкция**: `DEPLOY_NOW.md`

---

**Готово к деплою!** 🎉

Все файлы находятся в репозитории: https://github.com/hotwellkz/p041

